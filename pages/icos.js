...
export default function ICOs({ icos, banners }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (charge && charge.hosted_url) window.location.href = charge.hosted_url
    } catch (error) {
      console.error(error)
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/coinbase', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bannerData: bannerFormData, amount: bannerFormData.duration === '7' ? 99 : bannerFormData.duration === '14' ? 179 : 299 })
      })
      const charge = await response.json()
      if (charge && charge.hosted_url) window.location.href = charge.hosted_url
    } catch (error) {
      ...
      <main className="container mx-auto py-8 px-4">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex space-x-4">
            <button onClick={() => { setSelectedPackage('ico'); setShowModal(true) }} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
              Submit ICO
            </button>
            <button onClick={() => { setSelectedPackage('banner'); setShowBannerModal(true) }} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
              Advertise Banner
            </button>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg w-full max-w-md p-6">
              ...
              <form onSubmit={handleBannerSubmit}>
                ...
                <div className="mb-4">
                  <label htmlFor="image" className="block text-gray-700 mb-2">Banner Image *</label>
                  <input type="file" id="image" className="w-full p-2 border border-gray-300 rounded" accept="image/*" required onChange={handleImageChange} />
                </div>
                <div className="flex justify-end space-x-2">
